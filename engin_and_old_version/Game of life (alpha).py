...
def colcall():
    ...
    if v_color == 1:
        col1.configure(background="#343645",fg="#dfdfed")
        col2.configure(background="#343645",fg="#dfdfed")
        ...
        try:
            ...
            pass
        check1.configure(background="#343645",foreground="#dfdfed")
        ...
        col1.configure(background="#dfdfed",fg="#171717")
        col2.configure(background="#dfdfed",fg="#171717")
...
color_b = IntVar()
col1 = tk.Radiobutton(mode, text = 'Dark_Blue', variable = color_b, value = 1, command= colcall)#  , command = colcall
col1.grid(column = 0, row = 1,padx= 4 , pady= 4, sticky = tk.W)
col1.configure(background=coler)
col2 = tk.Radiobutton(mode, text="Light", variable= color_b, value= 2, command= colcall)
...
frame2.grid(column = 0, row = 0, padx= 7 , pady= 3)
...
slide_ch = tk.IntVar()
check1 = tk.Checkbutton(frame2, text = "Auto slide(unrecommended)", variable = slide_ch)
# check1.select()
...
def main_def():
    ...
    if fuck == 1:
        ...
        frame3.grid(column = 0, row = 1, padx= 7 , pady= 2)
        ...
        frame4.grid(column=1, row=1 ,padx=7, pady=3)
        ...
        strtime = str((fi-st))
        time_sh = tk.Label(frame4, text="Process time : {}".format(strtime), relief= "groove",background=coler,width=(len(strtime)+12),height=2,border=2)
        time_sh.grid(column=0, row=2,pady= 18,padx= 9, columnspan=1)
        ...
        def startgame():
            ...
            if slide == 1:
                stage_b.destroy()
                setting_t = int(number_t.get())
                while pause_ch == 1:
                    def paus():
                        global pause_ch
                        if pause_ch == 0:
                            pause_ch = 1
                            pause_b.configure(text=" pause ")
                        else:
                            pause_ch = 0
                            pause_b.configure(text=" unpause ")
                    if counter10 == 1:
                        pause_b = ttk.Button(frame4, text=" pause ", command= paus)
                        pause_b.grid(column=1, row= 1 , padx= 8 , pady= 4)
                    ...
                    strtime = str((fi-st))
                    time_sh.configure(text="Process time : {}".format(strtime))
                    stage.configure(text="in step {}".format(counter10))
                    counter10 +=1
                    time.sleep(setting_t)
                ...
                # globals("i_{}".format()) in first time and add each of them to the list
                # in each round for index,name in enumerate(locations):
                #                   if locations[name] == 1:
                #                       list[index].configure or globals....
